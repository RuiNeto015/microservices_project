...
public class UserPgRepositoryImpl implements IUserRepository {
    ...
    public void create(User user) throws DatabaseException {
        Optional<UserPg> userById = this.userRepository.findById(user.getId());

        if (userById.isPresent()) {
            throw new DatabaseException("Duplicate id violation.");
        }

        UserPg userByEmail = this.userRepository.findByEmail(user.getEmail());
    ...
    public void update(User user) throws DatabaseException {
        Optional<UserPg> matchingUser = this.userRepository.findById(user.getId());
    ...
    public List<User> findAll() {
        List<UserPg> databaseUsers = (List<UserPg>) this.userRepository.findAll();
        List<User> domainUsers = new ArrayList<>();
        for (UserPg u : databaseUsers) {
