...
public class UserMongoRepositoryImpl implements IUserRepository {
    ...
    @Autowired
    public UserMongoRepositoryImpl(UserRepositoryMongodb userRepository, ReviewRepositoryMongodb reviewRepository,
                                   UserMapper mapper) {
    ...
    public void create(User user) throws DatabaseException {
        ...
        UserMongo userByEmail = this.userRepository.findByEmail(user.getEmail());
    ...
    public void update(User user) throws DatabaseException {
        Optional<UserMongo> matchingUser = this.userRepository.findById(user.getId());
    ...
    public List<User> findAll() {
        List<UserMongo> databaseUsers = (List<UserMongo>) this.userRepository.findAll();
        List<User> domainUsers = new ArrayList<>();
        for (UserMongo u : databaseUsers) {
            domainUsers.add(this.mapper.toDomainObject(u));
        }
        return domainUsers;
    }
}
